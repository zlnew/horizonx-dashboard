...
const initialValues = computed(() => ({
  ...
  scope: props.rule?.scope ?? AlertScope.GLOBAL,
  server_id: props.rule?.server_id ?? '',
  app_id: props.rule?.app_id != null ? String(props.rule.app_id) : '',
  source: props.rule?.source ?? AlertSource.METRIC,
  metric_path: props.rule?.metric_path ?? '',
  operator: props.rule?.operator ?? '',
  threshold: props.rule?.threshold != null ? String(props.rule.threshold) : '',
  target_status: props.rule?.target_status ?? '',
  for_duration: props.rule?.for_duration != null ? String(props.rule.for_duration) : '0',
  severity: props.rule?.severity ?? AlertSeverity.WARNING,
...
watch(
  ...
  (open) => {
    ...
    if (initialValues.value.scope === AlertScope.APP) {
      loadApplications()
    }
  }
)
...
const onSubmit = handleSubmit(async (formValues) => {
  ...
  try {
    const res = await ruleStore.updateRule(props.rule.id, buildPayload(formValues))
    toast.success(res.message ?? 'Alert rule updated successfully')
    emit('success')
    close()
  } catch (error) {
    const fetchError = error as Error
  ...
  >
    <DialogContent class="sm:max-w-xl">
      <DialogHeader>
        ...
        <DialogDescription> Update the rule conditions and severity. </DialogDescription>
      </DialogHeader>
