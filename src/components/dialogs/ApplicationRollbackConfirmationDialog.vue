...
const rollbackApp = async (closeDialog: () => void) => {
  if (!applicationStore.selectedApplication?.id) {
    return
  }

  try {
    const res = await applicationStore.rollbackApplication(applicationStore.selectedApplication.id)
    if (res.message) {
